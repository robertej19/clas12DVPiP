...
def plot_2dhist(x_data,y_data,var_names,ranges,colorbar=True,
            ...
            filename="ExamplePlot",units=["",""],extra_data=None,xaxis_lines=None,yaxis_lines=None,xbq2plot=False,i=-1,j=1,
            y_bin_edges=None,x_bin_edges=None,ix=1,jx=1,rect_color="red",ax=None):
    ...
    # Use ax.hist2d instead of plt.hist2d
    h, xedges, yedges = np.histogram2d(x_data, y_data, bins=[x_bins, y_bins], range=[[xmin, xmax], [ymin, ymax]])
    pcm = ax.pcolormesh(xedges, yedges, h.T, norm=mpl.colors.LogNorm())

    # Use ax.set_title instead of plt.title
    #ax.set_title(plot_title)

    # Use ax.vlines and ax.hlines instead of plt.vlines and plt.hlines
    for xbin in xaxis_lines:   
        if xbq2plot:
            ax.vlines(xbin, ymin=ymin, ymax=ymax, color='lightgray')
            ax.vlines(xbin, ymin=np.max([bottom_line(xbin), 1]), ymax=top_line(xbin), color='r')
        else:
            ax.vlines(xbin, ymin=ymin, ymax=ymax, color='r')

    for ybin in yaxis_lines:
        print(ybin)
        if xbq2plot:
            ax.hlines(ybin, xmin=xmin, xmax=xmax, color='lightgray')
            ax.hlines(ybin, xmin=left_line(ybin), xmax=right_line(ybin), color='r')
        else:
            ax.hlines(ybin, xmin=xmin, xmax=xmax, color='r')

    # Use ax.plot instead of plt.plot
    if xbq2plot:
        ax.hlines(1, xmin=left_line(1), xmax=right_line(1), color='orange', linewidth=5)
        x_range = np.linspace(left_line(1), xmax, 1000)
        yvals = top_line(x_range)
        ax.plot(x_range, yvals, 'b', linewidth=5)
        x_range2 = np.linspace(right_line(1), xmax, 1000)
        yvals_bottom = bottom_line(x_range2)
        ax.plot(x_range2, yvals_bottom, 'k', linewidth=5)
        #set x axis limits
        ax.set_xlim(left_line(1),xmax)
        ax.set_ylim(1,ymax)
    

    # Use ax.add_patch instead of plt.gca().add_patch
    if xbin_space*ybin_space > -100:
        rect = Rectangle((x_bin_edges[xbin_space-ix], y_bin_edges[ybin_space-jx]), x_bin_edges[xbin_space-ix+3]-x_bin_edges[xbin_space-ix], y_bin_edges[ybin_space-jx+3]-y_bin_edges[ybin_space-jx], linewidth=1, edgecolor=rect_color, facecolor=rect_color,alpha=0.3)
        ax.add_patch(rect)
...
# # 
        ...
        for i in range(len(xb_bin_edges)-1):
            for j in range(len(q2_bin_edges)-1):
                ...
                bin_rec_df = df[(df[prefix+"xB"] > xb_bin_edges[i]) & 
                            ...
                            (df[prefix+"Q2"] > q2_bin_edges[j]) & 
                            (df[prefix+"Q2"] < q2_bin_edges[j+1]) &
                            (df[prefix+"t1"] > t_bin_edges[i]) & 
                            (df[prefix+"t1"] < t_bin_edges[i+1]) & 
                            (df[prefix+"phi1"] > phi_bin_edges[j]) & 
                            (df[prefix+"phi1"] < phi_bin_edges[j+1])
                            ]
                if len(bin_rec_df) > 1:
                            ...
                            for jx in jxs:
                                for px in ixs:
                                    for tx in jxs:
                                        ...
                                        # # plot_title = "$x_B$ vs $Q^2$, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}, N={}, $P${}%".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1], N_found,bin_purity)
                                        # plot_title = "$x_B$ vs $Q^2$, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1])

                                        # plot_2dhist(bin_rec_df[prefix+"xB"],bin_rec_df[prefix+"Q2"],["$x_B$","$Q^2$"],[[0.05,0.9,200],[0.5,12,200]],colorbar=True,xaxis_lines=xb_bin_edges,yaxis_lines=q2_bin_edges,
                                        #             plot_title=plot_title,units=["None","GeV$^2$"],xbq2plot=True,saveplot=False,pics_dir=out_dir,i=i,j=j,
                                        #             y_bin_edges=q2_bin_edges,x_bin_edges=xb_bin_edges,ix=ix,jx=jx,rect_color=rect_color)
                                        
                                        # #sys.exit()
                                        # #plot_title = "$\phi$ vs Momentum Transfer t, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}, N={}, $P$={}%".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1],N_found, bin_purity)
                                        # plot_title = "$\phi$ vs Momentum Transfer t, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1])

                                        # plot_2dhist(bin_rec_df[prefix+"phi1"],bin_rec_df[prefix+"t1"],["$\phi$","t"],[[0,360,100],[0,1.8,100]],colorbar=True,xaxis_lines=phi_bin_edges,yaxis_lines=t_bin_edges,
                                        #         plot_title=plot_title,units=["None","GeV$^2$"],saveplot=False,pics_dir=out_dir,figsize=(30,18),i=i,j=j,
                                        #         y_bin_edges=t_bin_edges,x_bin_edges=phi_bin_edges,ix=ix,jx=jx,rect_color=rect_color)

                                        plt.rcParams["font.size"] = "30"

                                        fig, axs = plt.subplots(1, 2, figsize=(18,10))

                                        # Create the title for the first plot
                                        plot_title = "$x_B$ vs $Q^2$, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1])

                                        # Make the first plot
                                        plt_1 = plot_2dhist(bin_rec_df[prefix+"xB"],bin_rec_df[prefix+"Q2"],["$x_B$","$Q^2$"],[[0.05,0.9,200],[0.5,12,200]],colorbar=True,xaxis_lines=xb_bin_edges,yaxis_lines=q2_bin_edges,
                                                    plot_title=plot_title,units=["None","GeV$^2$"],xbq2plot=True,saveplot=False,pics_dir=out_dir,i=i,j=j,
                                                    y_bin_edges=q2_bin_edges,x_bin_edges=xb_bin_edges,ix=ix,jx=jx,rect_color="blue", ax=axs[0])


                                        # Create the title for the second plot
                                        plot_title = "$\phi$ vs Momentum Transfer t, Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1])

                                        # Make the second plot
                                        plot_2dhist(bin_rec_df[prefix+"phi1"],bin_rec_df[prefix+"t1"],["$\phi$","t"],[[0,360,100],[0,1.8,100]],colorbar=True,xaxis_lines=phi_bin_edges,yaxis_lines=t_bin_edges[:-1],
                                                    plot_title=plot_title,units=["None","GeV$^2$"],saveplot=False,pics_dir=out_dir,figsize=(30,18),i=i,j=j,
                                                    y_bin_edges=t_bin_edges,x_bin_edges=phi_bin_edges,ix=tx,jx=px,rect_color="blue", ax=axs[1])

                                        # Display the plot
                                        #plt.tight_layout()
                                        #plot_title_sup = "Sim. {}, {}<$x_B$<{},{}<$Q^2$<{}".format(title_prefix,xb_bin_edges[i],xb_bin_edges[i+1],q2_bin_edges[j],q2_bin_edges[j+1])
                                        plot_title_sup = "3x3x3x3 Bin Migration Unfolding"
                                        fig.suptitle(plot_title_sup, fontsize=30,y=0.85)

                                        # Display the plot
                                        plt.tight_layout(rect=[0, 0.03, 1, 0.95]) # Adjust the layout to make room for the title
                                        #plt.show()
                                        #save the figure
                                        plt.savefig("bin_kerneling/bin_migration_{}_{}_{}_{}.png".format(ix,jx,px,tx))
                        sys.exit()

                        # fig, ax = plot_2dhist(bin_rec_df[prefix+"xB"],bin_rec_df[prefix+"Q2"],["$x_B$","$Q^2$"],[[0.05,0.9,200],[0.5,12,200]],colorbar=True,xaxis_lines=xb_bin_edges,yaxis_lines=q2_bin_edges,
                        #                             plot_title=plot_title,units=["None","GeV$^2$"],xbq2plot=True,saveplot=False,pics_dir=out_dir)

                        # # Create a Rectangle patch
                        # rect = Rectangle((xb_bin_edges[i-1], q2_bin_edges[j-1]), xb_bin_edges[i+2]-xb_bin_edges[i-1], q2_bin_edges[j+2]-q2_bin_edges[j-1], linewidth=1, edgecolor='r', facecolor='none')

                        # # Add the patch to the Axes
                        # ax.add_patch(rect)

                        # fig, ax = plot_2dhist(bin_rec_df[prefix+"phi1"],bin_rec_df[prefix+"t1"],["$\phi$","t"],[[0,360,100],[0,1.8,100]],colorbar=True,xaxis_lines=phi_bin_edges,yaxis_lines=t_bin_edges,
                        #                         plot_title=plot_title,units=["None","GeV$^2$"],saveplot=False,pics_dir=out_dir,figsize=(30,18))

                        # # Create a Rectangle patch
                        # rect = Rectangle((phi_bin_edges[i-1], t_bin_edges[j-1]), phi_bin_edges[i+2]-phi_bin_edges[i-1], t_bin_edges[j+2]-t_bin_edges[j-1], linewidth=1, edgecolor='r', facecolor='none')
    ...
    if plot_sines:
        ...
        tp_1 = np.sin(df[prefix+"xB"]*3.14159/180*1500)#df[prefix+"t2"]
        tp_2 = np.sin(df[prefix+"Q2"]*3.14159/180*150)#df[prefix+"t2"]

        # # Normalize the W2 values to range between 0 and 1
        #norm = plt.Normalize(W2.min(), W2.max())
        names = ["xB","Q2"]
        for tp,name in zip([tp_1,tp_2],names):
            prefix = ""#"Gen"
            tp_norm = plt.Normalize(tp.min(), tp.max())
            fig, axs = plt.subplots(1, 2, figsize=(16, 8), sharey=True)

            # Left subplot for "gen"
            axs[0].scatter(df["Gen"+"xB"], df["Gen"+"Q2"], marker="o", c=tp, cmap='viridis', norm=tp_norm)
            axs[0].set_xlim([0, .8])
            axs[0].set_ylim([.5, 11])
            ...
            axs[1].scatter(df["xB"], df["Q2"], marker="o", c=tp, cmap='viridis', norm=tp_norm)
            axs[1].set_xlim([0, .8])
            axs[1].set_ylim([.5, 11])
            axs[1].set_title('Reconstructed Distribution')
            axs[1].set_xlabel('$x_B$')
            axs[1].set_ylabel('')

            plt.tight_layout()
            #plt.show()
            plt.savefig("sine_plots_{}.png".format(name))
            plt.close()

        prefix = "Gen"#"Gen"

        tp_1 = np.sin(df[prefix+"phi1"]*3.14159/180*5)#df[prefix+"t2"]
        tp_2 = np.sin(df[prefix+"t1"]*3.14159/180*1000)#df[prefix+"t2"]
