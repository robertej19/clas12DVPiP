...
def convert_rec_to_pandas(args):
    ...
    while starting_entry < args.entry_stop:
        ...
        if args.rad:
            ...
            df.loc[:, 'GenPie'] = physics.getEnergy(pi0, PhysicsConstants.neutral_pion_mass)


       
            ###

            Ppt = physics.mag([['GenPpx'], df['GenPpy'], 0])
            ...
            df.loc[:,'GenopeningAngle'] = angle(gam, pi0)
            ...
            # VmissPi0 = [-df["Epx"] - df["Ppx"], -df["Epy"] -
            #         df["Ppy"], pbeam - df["Epz"] - df["Ppz"]]
            
            # VmissP = [-df["Epx"] - df["Gpx"] - df["Gpx2"], -df["Epy"] -
            #             df["Gpy"] - df["Gpy2"], pbeam - df["Epz"] - df["Gpz"] - df["Gpz2"]]
            
            # Vmiss = [-df["Epx"] - df["Ppx"] - df["Gpx"] - df["Gpx2"],
            #             -df["Epy"] - df["Ppy"] - df["Gpy"] - df["Gpy2"],
            #             pbeam - df["Epz"] - df["Ppz"] - df["Gpz"] - df["Gpz2"]]
...
if __name__ == "__main__":
    ...
    plt.hist(np.sqrt(df['GenMM2_epgg']), bins=50)
    plt.show()
    ...
    print(df[x])
    print(df[y])
    plt.hist2d(df[x], df[y], bins=(200, 200), norm=mpl.colors.LogNorm())
    plt.colorbar()
    plt.xlabel('phi1')
    plt.ylabel('phi2')
    plt.title('2D Histogram of phi1 and phi2')
    plt.show()
